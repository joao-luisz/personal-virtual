...
def criar_banco_dados():
    ...
    """
    conexao = None  # Inicializa a variável para evitar o erro
    try:
        ...
        if conexao:  # Verifica antes de fechar
            conexao.close()
...
def validar_email_ou_telefone(email_ou_telefone):
    ...
    """
    if not email_ou_telefone or not isinstance(email_ou_telefone, str):
        print("Erro: O campo e-mail/telefone não pode estar vazio.")
        return False
...
def cadastrar_usuario(nome_usuario, email_ou_telefone, senha, nome_preferido, idade, peso, altura, nivel_experiencia, objetivo):
    ...
    """
    conexao = None  # Inicializa a variável
...
def salvar_treino(usuario_id, exercicios):
    ...
    try:
        conexao = sqlite3.connect("data/users.db")
        cursor = conexao.cursor()
        exercicios_str = ", ".join(exercicios) if exercicios else ""
        cursor.execute("""
