...
import sys
import os
from modules.workout_recommendation import recomendar_exercicios_mega_gym
from modules.user_management import (
    recuperar_informacoes_usuario,
    salvar_treino,
    salvar_progresso,
    recuperar_historico_progresso,
    exibir_meus_treinos,
)
from modules.translations import traducoes  # Importar o dicionário de traduções
...
def iniciar_chatbot(usuario_id=None, genero="Masculino", dias_disponiveis=3, equipamentos="Sim"):
    ...
    Recebe parâmetros diretamente da interface gráfica ou do terminal.
    """
    ...
    salvar_treino(
        usuario_id,
        [exercicio.split(" (")[0] for info in planilha_traduzida.values() for exercicio in info["exercicios"]]
    )
